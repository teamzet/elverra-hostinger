  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user) {
        ...
        setTimeout(async () => {
          try {
            // Check user role from context (updated by useAuth)
            if (isAdmin) {
              ...
              try {
                const membershipResponse = await fetch(`/api/memberships/${data.user.id}`);
                const membershipData = membershipResponse.ok ? await membershipResponse.json() : null;
